...
import { addSaveJuliaPNGBtnListeners, setupPreviewCPURenderBtn, setupPreviewCenterOriginBtn, setupPreviewDownload, } from './ui/juliaDownload.js';
...
const mandelContext = new MandelContext(canvasMandel, canvasMandel2d, window.innerWidth / 2, window.innerHeight, { x: 0, y: 0 }, fragmentShaderTextMandel, nrIterations);
...
const juliaContext = new JuliaContext(canvasJulia, canvasJulia2d, window.innerWidth / 2, window.innerHeight, {
    x: mandelContext.vp.vWidth,
    y: 0,
}, fragmentShaderTextJulia, nrIterations);
...
const juliaDrawingContext = new JuliaContext(juliaDrawingCanvas, juliaDrawingCanvas2d, 3092, 1920, { x: 0, y: 0 }, fragmentShaderTextJulia, nrIterations);
juliaDrawingContext.setProgressBarElement('cpu-rendering-progress-bar', 'cpu-rendering-progress-bar-time');
const juliaPreviewCanvas = document.getElementById('download-preview-canvas');
const juliaPreviewCanvas2d = document.getElementById('download-preview-canvas-2d');
const juliaPreviewContext = new JuliaContext(juliaPreviewCanvas, juliaPreviewCanvas2d, window.innerWidth * 0.55, window.innerHeight * 0.55, { x: window.innerWidth * 0.225, y: window.innerHeight * 0.225 }, fragmentShaderTextJulia, nrIterations);
...
setupPreviewDownload(juliaDrawingContext, juliaPreviewContext, 'download-julia-btn', 'download-resolution-x', 'download-resolution-y', juliaCommunityCheckboxId);
...
addSaveJuliaPNGBtnListeners(juliaContext, juliaDrawingContext, 'save-julia-png-btn', juliaPreviewContext, juliaPreviewContainerId, 'shortcut-save-checkbox');
