...
export class FractalManager {
    ...
    juliaFollowsMouse;
    movingRandom;
    constructor(context1, context2, juliaCenterXInputId, juliaCenterYInputId, juliaPreviewContext, isPreviewVisible) {
    ...
    setCurrentJuliaCenter(xCoord, yCoord) {
    ...
    addJuliaCoordInputListeners = (juliaCenterXInputId, juliaCenterYInputId) => {
        this.juliaXCoordInput = document.getElementById(juliaCenterXInputId);
        this.juliaYCoordInput = document.getElementById(juliaCenterYInputId);
        // Coord input
    ...
    addUpdateJuliaOnMouseMove() {
        this.mandelContext.canvas.addEventListener('mousemove', (evt) => {
            if (!this.mandelContext.indicatorFollowsMouse || this.mandelContext.panningObject.panningCanvas)
                return;
    ...
    addPausingUpdateJulia(indfollowmouseShortcutCheckboxId, randmoveShortcutCheckboxId) {
        const indfollowmouseShortcutCheckbox = (document.getElementById(indfollowmouseShortcutCheckboxId));
        const randmoveShortcutCheckbox = document.getElementById(randmoveShortcutCheckboxId);
    ...
    tryParseParamsFromFilename = (filename) => {
        // Extract parameters
        let params = filename.split('_').slice(1); // Split into attributes, remove "JuliaSet"-prefix
        if (params.length < 12)
            return { parsedSuccessfully: false }; // Not enough params
    ...
    updateRenderFractals = (params, juliaPreviewContext, juliaPreviewContainerId) => {
    ...
    randomMovement() {
        ...
        async function innerLoop(frameNr, fractalManager) {
