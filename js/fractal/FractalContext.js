...
import { getCanvasRenderingContext2D, getWebGL2RenderingContext, canvasMoveEvent } from '../utils/utils.js';
...
export class FractalContext {
    ...
    updateProgressBar = (progress) => {
        ...
        this.progressBar.timeField.innerText = `${(elapsedTime / 1000).toFixed(2)} | remaining: ~ ${(estimatedRemainingTime / 1000).toFixed(0)}`;
    ...
    addPanZoomToCanvas = (canvas) => {
        ...
        canvas.addEventListener('mousemove', (evt) => {
            if (!this.panningObject.panningCanvas)
                return;
            let vp = this.vp;
            // Pan canvas
    ...
    dispatchCanvasMoveEvent = () => {
        ...
        if (!this.cpuRendering)
            this.canvas2d.dispatchEvent(canvasMoveEvt);
        else
            this.canvas2d.dispatchEvent(canvasMoveEvt);
    };
    ...
    addDoubleClickCenterPoint = () => {
        ...
        const handleLastClick = (evt) => {
            if (performance.now() - lastClick.time <= DOUBLE_CLICK_INTERVAL &&
    ...
    addColorInputListener = (colorPickerId, isVisible = () => true) => {
        this.colorInput = document.getElementById(colorPickerId);
    ...
    addEscapeRadiusInputListener = (escapeRadiusInputId, isVisible = () => true) => {
        this.escapeRadiusInput = document.getElementById(escapeRadiusInputId);
    ...
    addExponentInputListener = (exponentInputId, isVisible = () => true) => {
        this.exponentInput = document.getElementById(exponentInputId);
    ...
    addNrIterationsInputListener = (nrIterationsInputId, isVisible = () => true) => {
        this.nrIterationsInput = document.getElementById(nrIterationsInputId);
    ...
    addColorSettingsInputs = (colorDropdownId, isVisible = () => true) => {
        ...
        this.colorSettingsInputs.forEach((input) => input.addEventListener('input', (evt) => {
            if (!isVisible())
                return;
    ...
    tryResizeCanvasMediumSize = () => {
        ...
        if (width < window.innerWidth * 0.5 && height < window.innerHeight * 0.5) {
            ...
            width -= 10;
            height = width / ratio;
            this.resizeCanvas(window.innerWidth * 0.5 - width * 0.5, window.innerWidth * 0.5 + width * 0.5, window.innerHeight * 0.5 - height * 0.5, window.innerHeight * 0.5 + height * 0.5);
        }
    };
}
